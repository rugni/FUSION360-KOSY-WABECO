...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  var middle = {"x": (x-start.x)/2, "y": (y-start.y)/2}; // WABECO MOD: stores the distance between the middle between the start and and point
  var lengthOpposite = Math.sqrt(Math.pow(middle.x,2)+Math.pow(middle.y,2)); // WABECO MOD: Calculate the distance between middle and end point using Pythagoras
  var lengthAdjacent = Math.sqrt(Math.pow(cx-(start.x+middle.x),2)+Math.pow(cy-(start.y+middle.y),2)); //WABECO MOD: calculate the distance between middle and center using Pythagoras
  var angleRadians = Math.atan(lengthOpposite/lengthAdjacent) * 2; //WABECO MOD: calculate angle in radians using tangens
  var angleDegrees = angleRadians*180/Math.PI; //WABECO MOD: convert angle to degrees
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      // writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed)); // this is the old method
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      xOutput.reset();
      yOutput.reset();
      writeBlock(gMotionModal.format(clockwise ? 2:3), dOutput.format(angleDegrees,0), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0),zOutput.format(z), feedOutput.format(feed));
      // writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed)); // this is the old method
      break;
    default:
      linearize(tolerance);
    }
